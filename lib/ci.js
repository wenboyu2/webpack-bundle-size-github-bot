...
const baseUrl = isJenkins ? `https://${config.CI_USERNAME}:${config.CI_TOKEN}@jenkins.pod.box.net` : '';

function getTravis(url, options = {}) {
    return axios.get(url, _extends({}, options, {
        headers: {
...
async function getPrBundleSizes(prNumber) {
    return isJenkins ? getJenkinsPrBundleSizes(prNumber) : getTravisPrBundleSizes(prNumber);
}

async function getTravisLatestBuildIdOfPr(prNumber) {
    const slug = encodeURIComponent(`${config.GITHUB_OWNER}/${config.GITHUB_REPO}`);

    const res = await getTravis(`https://api.travis-ci.org/repo/${slug}/builds?event_type=pull_request&state=passed&sort_by=finished_at:desc&limit=100`);

    const latestBuild = res.data.builds.find(build => parseInt(build.pull_request_number, 10) === parseInt(prNumber, 10));
...
async function getJenkinsLatestBuildOfPr(prNumber) {
    const res = await axios.get(`${baseUrl}/blue/rest/organizations/jenkins/pipelines/EndUserApp/pipelines/ContinuousIntegration/pipelines/PullRequest/runs/`);
    ...
    if (latestBuild.result !== 'SUCCESS') {
        throw new Error(`PR #${prNumber} latest build ${buildId} is not successful`);
    }
...
async function getJenkinsPrBundleSizes(prNumber) {
    ...
    const res = await axios.get(`${baseUrl}/blue/rest/organizations/jenkins/pipelines/EndUserApp/pipelines/ContinuousIntegration/pipelines/PullRequest/runs/${jobId}/log`);
    return parseJobLog(res.data);
}

function cleanString(value) {
    return value.replace(/[\x00-\x09\x0B-\x0C\x0E-\x1F\x7F-\x9F]/g, '').replace(/\[\d+m/g, '');
}
