...
class GithubClient {
    ...
    async getMergeBaseCommit(prNumber) {
        ...
        log.info(`getMergeBaseCommit #${prNumber} ==> ${res.data.merge_base_commit.sha}`);
    ...
    async getPrevPrNumber(prNumber) {
        ...
        const immediateCommitPrNumber = await this.getCommitPrNumber(commitHash);
    ...
    async getExistingBotComment(prNumber) {
        ...
        const botComment = comments.find(({ body }) => body.indexOf(title) !== -1 || body.indexOf(legacyTitle) !== -1);
    ...
    async postPullRequestComment(message, prNumber) {
        ...
        if (botComment) {
            ...
            log.info(`postPullRequestComment #${prNumber} existing commentId ==> ${botCommentId}`);
            log.info(`postPullRequestComment #${prNumber} ${botCommentId} areCommentsIdentical ==> ${areCommentsIdentical}`);
